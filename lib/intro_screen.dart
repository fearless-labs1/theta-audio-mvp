...
class _IntroScreenState extends State<IntroScreen> {
  ...
  Future<void> _tryFallbackIntroFormat() async {
    try {
      ...
      _introVideoController = VideoPlayerController.asset('assets/video/intro_video.mov');
  ...
  Future<void> _waitForFirstFrame(VideoPlayerController controller, String videoName) async {
    ...
    while (attempts < maxAttempts) {
      ...
      if (isPlaying && position.inMilliseconds > 0) {
        debugPrint('✅ First frame rendered ($videoName) - position: ${position.inMilliseconds}ms');
        return;
      }
      ...
      if (attempts % 10 == 0) {
        debugPrint('   Still waiting... attempt $attempts, isPlaying: $isPlaying, position: ${position.inMilliseconds}ms');
      }
    }
  ...
  void _startIntroTimeout() {
    final duration = _introVideoController?.value.duration ?? const Duration(seconds: 30);
  ...
  void _startPlaybackMonitor() {
    ...
    _playbackMonitorTimer = Timer.periodic(const Duration(milliseconds: 500), (timer) {
      if (!mounted) {
        timer.cancel();
        return;
      }
      ...
      if (_showingIntro && _introVideoController != null && !_introCompleted) {
        ...
        if (isPlaying && currentPos == _lastIntroPosition && currentPos.inMilliseconds > 0) {
          _stuckFrameCount++;
          debugPrint('⚠️ Intro video may be stuck - count: $_stuckFrameCount, pos: ${currentPos.inMilliseconds}ms');
          
          if (_stuckFrameCount >= 6) { // 3 seconds of no progress
      ...
      if (_showingInstruction && _instructionVideoController != null && !_instructionCompleted) {
        final currentPos = _instructionVideoController!.value.position;
        final isPlaying = _instructionVideoController!.value.isPlaying;
        
        if (isPlaying && currentPos == _lastInstructionPosition && currentPos.inMilliseconds > 0) {
          _stuckFrameCount++;
          debugPrint('⚠️ Instruction video may be stuck - count: $_stuckFrameCount');
  ...
  void _checkIntroProgress() {
    ...
    if (duration.inMilliseconds > 0 && 
        position.inMilliseconds >= duration.inMilliseconds - 100) { // 100ms tolerance
      debugPrint('✅ INTRO VIDEO COMPLETE (position: ${position.inMilliseconds}ms, duration: ${duration.inMilliseconds}ms)');
      _switchToInstructionVideo();
      return;
    }
  ...
  void _startInstructionTimeout() {
    final duration = _instructionVideoController?.value.duration ?? const Duration(seconds: 30);
    final timeoutDuration = duration + const Duration(seconds: 5);
    
    debugPrint('⏱️ Instruction timeout set for ${timeoutDuration.inSeconds} seconds');
  ...
  Widget _buildVideoContent() {
    ...
    if (_showingInstruction && _isInstructionInitialized && _instructionVideoController != null) {
      return Center(
        child: AspectRatio(
